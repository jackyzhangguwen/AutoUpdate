...
// 绘制标题栏所需的图像集（每个图像宽度为 m_iTitleBarImageWidth 象素，
// 按正常、失去焦点的顺序排列，总宽度不小于 2 * m_iTitleBarImageWidth 象素）
...
// 绘制按钮所需的图像集（每个图像宽度为 m_iButtonImageWidth 象素，
// 按正常、缺省、鼠标悬停、鼠标按下的顺序排列，
// 总宽度不小于 4 * m_iButtonImageWidth 象素）
...
// 绘制系统控制按钮（最小化、最大化、关闭等按钮）所需的图像集
// （每个图像宽度为 m_iSystemControlButtonImageWidth 象素，
// 按最小化、最大化、恢复、关闭、按钮按下状态的顺序排列，
// 总宽度不小于 8 * m_iSystemControlButtonImageWidth 象素）
...
HBRUSH CUISkinManager::OnCtlColor(CDC* pDC, 
								CWnd* pWnd, 
								UINT nCtlColor)
{
	if (pDC == NULL || 
...
// 以渐变方式填充矩形
void CUISkinManager::GradientFillRect(CWnd* pWnd,
									CDC* pDC, 
									CRect& rcFill, 
									ULONG nMode, 
									COLORREF crLeftTop, 
...
// 透明拉伸复制
void CUISkinManager::TransparentBlt(HDC hdcDest,
									int nXDest, 
									int nYDest, 
									int nWidthDest, 
									int nHeightDest, 
...
// 绘制窗口
void CUISkinManager::Paint(CWnd* pWnd, 
	CDC* pDC, 
	BOOL bFillClient, 
...
void CUISkinManager::Paint_Normal(CWnd* pWnd, 
								CDC* pDC, 
								BOOL bFillClient, 
								COLORREF crFillColor)
{
	...
	// 左部
	TransparentBlt(dcMemory.GetSafeHdc(),
					0, 
					0, 
					m_iTitleBarImageLeftPartWidth, 
					m_iTitleBarHeight, 
					hdcSrc, 
					0, 
					0, 
	...
	TransparentBlt(dcMemory.GetSafeHdc(),
					m_iTitleBarImageLeftPartWidth, 
					0,
					rcTitleBar.Width() - m_iTitleBarImageLeftPartWidth - m_iTitleBarImageRightPartWidth, 
					m_iTitleBarHeight,
					hdcSrc, 
					m_iTitleBarImageLeftPartWidth, 
					0,
	...
	// 右部
	TransparentBlt(dcMemory.GetSafeHdc(),
					rcTitleBar.Width() - m_iTitleBarImageRightPartWidth, 
					0,
					m_iTitleBarImageRightPartWidth, 
					m_iTitleBarHeight, 
					hdcSrc, 
					m_iTitleBarImageLeftPartWidth + m_iTitleBarImageMiddlePartWidth, 
					0,
					m_iTitleBarImageRightPartWidth, 
					m_iTitleBarHeight, 
					TRANSPARENT_COLOR);
...
void CUISkinManager::Paint_XP(CWnd* pWnd, 
							CDC* pDC, 
							BOOL bFillClient, 
							COLORREF crFillColor)
{
...
// 绘制对话框边框，对话框具有通用样式：
// 蓝色框线、下方有按钮工具条
void CUISkinManager::PaintDialogBorder(CWnd* pWnd, 
									   CDC* pDC)
{
	if (pWnd == NULL || 
		pWnd->m_hWnd == NULL) {
	...
	// 对话框底部按钮工具条高度（象素）
	const int BOTTOM_COMMAND_BUTTON_BAR_HEIGHT = 45;
	CRect rcWnd;
	CRect rc;

	pWnd->GetClientRect(&rcWnd);
	rcWnd.DeflateRect(m_iLeftMargin, 
					m_iTitleBarHeight + m_iTopMargin, 
					m_iLeftMargin + 1,
					m_iTopMargin + 1);

	// 画水平分隔线
	rc.SetRect(rcWnd.left, 
				rcWnd.bottom - BOTTOM_COMMAND_BUTTON_BAR_HEIGHT, 
				rcWnd.right, 
				rcWnd.bottom - BOTTOM_COMMAND_BUTTON_BAR_HEIGHT + 2);
