...
//
int Internet::InternetGetURL(const char* sURL, 
							...
							HWND hProgressWindow)
{
	...
	if (!(iFlags & INTERNET_CONNECTION_PROXY)) {
		hInternet = InternetOpenA(sAgent, 
	...
	if (hInternet) {
		...
		// Get URL
		if (!(hConnect = InternetOpenUrlA (hInternet, 
											sURL, 
											sHeader, 
											lstrlenA(sHeader), 
		...
		do {
			if (bInternetGetURLUserBreak) { break; }
			...
			if (iReadBytes <= 0) {
				// Condition of iReadBytes=0 indicate EOF. Stop.
				break;
			}
