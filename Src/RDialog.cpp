...
CRDialog::CRDialog(LPCTSTR lpszTemplateName, 
				   CWnd* pParentWnd /*=NULL*/)
	: CDialog(lpszTemplateName, pParentWnd)
{
}

CRDialog::CRDialog(UINT nIDTemplate, 
				   CWnd* pParentWnd /*=NULL*/)
...
BOOL CRDialog::OnInitDialog()
{
	...
	// SetWindowPos(&CWnd::wndTopMost, -1, -1, -1, -1, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
...
// 处理命令消息
BOOL CRDialog::OnCmdMsg(UINT nID, 
						int nCode, 
						void* pExtra, 
						AFX_CMDHANDLERINFO* pHandlerInfo)
{
	if (nID == IDC_BUTTON_MINIMIZE) {
		OnBtnClickedMinimize();
	} else if (nID == IDC_BUTTON_MAXIMIZE) {
		OnBtnClickedMaximize();
...
//	RDIALOG_UI_MESSAGE_MAP
	ON_BN_CLICKED(IDC_BUTTON_MINIMIZE, OnBtnClickedMinimize)
	ON_BN_CLICKED(IDC_BUTTON_MAXIMIZE, OnBtnClickedMaximize)
	ON_BN_CLICKED(IDC_BUTTON_CLOSE, OnBtnClickedClose)
END_MESSAGE_MAP()
...
void CRDialog::OnSize(UINT nType, 
					int cx, 
					int cy)
{
...
void CRDialog::OnLButtonDown(UINT nFlags, 
							CPoint point)
{
...
// 点击最小化按钮
void CRDialog::OnBtnClickedMinimize()
{
	// 发送最小化窗口消息
...
// 点击最大化按钮
void CRDialog::OnBtnClickedMaximize()
{
...
// 点击关闭按钮
void CRDialog::OnBtnClickedClose()
{
	// 发送关闭窗口消息
