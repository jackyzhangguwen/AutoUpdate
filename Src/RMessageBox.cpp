...
BOOL CRMessageBox::OnInitDialog()
{
	...
	// 判断是否需要显示复选框
	if (!m_strOptionPromptMessage1.IsEmpty()) {
		m_chkOption1.SetWindowText(m_strOptionPromptMessage1);
		m_chkOption1.ShowWindow(SW_SHOW);
	} else {
		m_chkOption1.ShowWindow(SW_HIDE);
	}
	if (!m_strOptionPromptMessage2.IsEmpty()) {
		m_chkOption2.SetWindowText(m_strOptionPromptMessage2);
...
BEGIN_MESSAGE_MAP(CRMessageBox, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_OK, OnBtnClickedOk)
	ON_BN_CLICKED(IDC_BUTTON_CANCEL, OnBtnClickedCancel)
	ON_BN_CLICKED(IDC_BUTTON_ABORT, OnBtnClickedAbort)
	ON_BN_CLICKED(IDC_BUTTON_IGANORE, OnBtnClickedIganore)
	ON_BN_CLICKED(IDC_BUTTON_RETRY, OnBtnClickedRetry)
	ON_BN_CLICKED(IDC_BUTTON_CONTINUE, OnBtnClickedContinue)
	ON_BN_CLICKED(IDC_BUTTON_YES, OnBtnClickedYes)
	ON_BN_CLICKED(IDC_BUTTON_NO, OnBtnClickedNo)
END_MESSAGE_MAP()
...
void CRMessageBox::OnBtnClickedOk()
{
	m_iID = IDOK;
...
void CRMessageBox::OnBtnClickedCancel()
{
	m_iID = IDCANCEL;
...
void CRMessageBox::OnBtnClickedAbort()
{
	m_iID = IDABORT;
...
void CRMessageBox::OnBtnClickedIganore()
{
	m_iID = IDIGNORE;
...
void CRMessageBox::OnBtnClickedRetry()
{
	m_iID = IDRETRY;
...
void CRMessageBox::OnBtnClickedContinue()
{
	//	m_iID = IDCONTINUE;
...
void CRMessageBox::OnBtnClickedYes()
{
	m_iID = IDYES;
...
void CRMessageBox::OnBtnClickedNo()
{
	m_iID = IDNO;
...
void CRMessageBox::ResetMessageBox()
{
	...
	size = pDC->GetTextExtent(m_strPromptMessage);
	...
	// 显示提示信息
	m_stcPromptMessage.SetWindowText(m_strPromptMessage);
