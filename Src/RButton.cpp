...
void CRButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (CUISkinManager::m_iCurrentStyle == CUISkinManager::StyleSummer) {
		if (m_iButtonType == ButtonTypeMinimize) {
			// 绘制最小化窗口按钮
			DrawSystemControlButtonWithBitmap(pDC->GetSafeHdc(),
											rcButton, 
											(HBITMAP)CUISkinManager::m_bmSystemControlButtonImage, 
											m_bDown ? 16 : 0);
			return;
			...
			DrawItemWithBitmap(pDC->GetSafeHdc(), 
								rcButton, 
								(HBITMAP)CUISkinManager::m_bmButtonImage, 
								20, 
								32);
		} else if (m_bOver) {
			DrawItemWithBitmap(pDC->GetSafeHdc(), 
		...
		CUISkinManager::GradientFillRect(this, 
										pDC, 
										rcOver, 
		...
		// 外边框
		// 边框颜色
		CPen pen(PS_SOLID, 1, RGB(200, 210, 240));
...
//
// 以位图方式描绘按钮控件，要求位图宽度为32象素
// （左15 + 中2 + 右15，中间随着控件宽度拉伸）
...
//          按钮处于不同状态（正常、有焦点、
//          鼠标悬停、按下）时对应不同的起点位置
void CRButton::DrawItemWithBitmap(HDC hdcDest, 
								CRect& rc, 
								HBITMAP hBitMap, 
								UINT iBitMapHeight, 
								UINT cx)
{
	...
	TransparentBlt(hdcDest,
					0, 
					0, 
					CUISkinManager::m_iButtonImageLeftPartWidth, 
					rc.Height(),
					hdcSrc, 
					cx, 
					0, 
	...
	TransparentBlt(hdcDest, 
					...
					0
					, 
					rc.Width() - CUISkinManager::m_iButtonImageLeftPartWidth + CUISkinManager::m_iButtonImageRightPartWidth, 
					rc.Height(), 
					hdcSrc, 
					cx + CUISkinManager::m_iButtonImageLeftPartWidth, 
					0,
					CUISkinManager::m_iButtonImageMiddlePartWidth, 
					iBitMapHeight,
					CUISkinManager::TRANSPARENT_COLOR);

	TransparentBlt(hdcDest,
					rc.Width() - CUISkinManager::m_iButtonImageRightPartWidth, 
					0,
					CUISkinManager::m_iButtonImageRightPartWidth, rc.Height(),
					hdcSrc,
					cx + CUISkinManager::m_iButtonImageLeftPartWidth + CUISkinManager::m_iButtonImageMiddlePartWidth, 
					0,
					CUISkinManager::m_iButtonImageRightPartWidth, 
					iBitMapHeight,
					CUISkinManager::TRANSPARENT_COLOR);
...
// HBITMAP hBitMap：位图句柄
// UINT cx：从位图复制的起点位置
void CRButton::DrawSystemControlButtonWithBitmap(HDC hdcDest, 
												CRect& rc, 
												HBITMAP hBitMap, 
												UINT cx)
{
...
// 描绘边框
void CRButton::DrawRoundBorder(CDC *pDC, 
							   CRect rcBorder)
{
