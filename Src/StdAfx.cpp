...
// szFormat：日志格式
// ...：日志内容
void LOG(int iModuleID, 
		int iLevel, 
		const char* sFormat, ...)
{
	// 只记录级别在指定日志级别以上的日志
	if (iLevel < g_iLogLevel) { return;	}
	...
	try {
		...
		file.Open(GetAppDirectory() + "Log\\AutoUpdate.log",
					CFile::modeCreate | 
...
// 显示消息框
int RMessageBox(LPCTSTR lpszText, 
				UINT nType, 
				UINT nIDHelp)
{
	ASSERT(lpszText);
	...
	mb.m_strTitle = AfxGetApp()->m_pszAppName;
	mb.m_iType = nType;
	mb.m_strPromptMessage = lpszText;
	mb.DoModal();
	return mb.m_iID;
}
...
CString ResolvePath(const char* sPath)
{
	...
	memset(acBuffer, 0, MAX_PATH);
	// CSIDL_FONTS (0x0014)
	SHGetSpecialFolderPath(NULL, acBuffer, 0x0014, FALSE);
	sDestFilename.Replace("<FONTS>", acBuffer);

	memset(acBuffer, 0, MAX_PATH);
	// CSIDL_PROGRAM_FILES (0x0026)
	SHGetSpecialFolderPath(NULL, acBuffer, 0x0026, FALSE);
	sDestFilename.Replace("<PROGRAMFILES>", acBuffer);

	memset(acBuffer, 0, MAX_PATH);
	// CSIDL_PROGRAM_FILES_COMMON (0x002b)
	SHGetSpecialFolderPath(NULL, acBuffer, 0x002b, FALSE);
	sDestFilename.Replace("<COMMONFILES>", acBuffer);

	memset(acBuffer, 0, MAX_PATH);
	// CSIDL_PROFILE (0x0028)
	SHGetSpecialFolderPath(NULL, acBuffer, 0x0028, FALSE);
	...
	SHGetSpecialFolderPath(NULL, acBuffer, 0x0006, FALSE);
	...
	SHGetSpecialFolderPath(NULL, acBuffer, 0x001c, FALSE);
